...
BIGBLUEBUTTON_ROOT_DIR=~/home/dev/novinsystembbb2.3
AKKA_APPS_DIR=$BIGBLUEBUTTON_ROOT_DIR/akka-bbb-apps
COMMON_MESSAGE_DIR=$BIGBLUEBUTTON_ROOT_DIR/bbb-common-message
COMMON_WEB_DIR=$BIGBLUEBUTTON_ROOT_DIR/bbb-common-web
BIGBLUEBUTTON_WEB_DIR=$BIGBLUEBUTTON_ROOT_DIR/bigbluebutton-web
BIGBLUEBUTTON_WEB_EXPOLODED_DIR=$BIGBLUEBUTTON_ROOT_DIR/bigbluebutton-web/exploded
FSESL_CLIENT_DIR=$BIGBLUEBUTTON_ROOT_DIR/bbb-fsesl-client
AKKA_FSESL_DIR=$BIGBLUEBUTTON_ROOT_DIR/akka-bbb-fsesl
BIGBLUEBUTTON_HTML5_APP_DIR=$BIGBLUEBUTTON_ROOT_DIR/bigbluebutton-html5
...
LOCAL_PACKAGING_DIR=$BIGBLUEBUTTON_ROOT_DIR/build/packages-template/bbb-html5

#ARP
cd "$BIGBLUEBUTTON_HTML5_APP_DIR"
MASTER_BRANCH="$1"
sudo cp "$DESTINATION_DIR"/programs/server/assets/app/config/settings.yml /home
git add -A
git commit --allow-empty -m "bbb server commit"
git pull --no-edit
...
#ARP
cd "$COMMON_MESSAGE_DIR"
./deploy.sh
cd "$COMMON_WEB_DIR"
./deploy.sh
cd "$BIGBLUEBUTTON_WEB_DIR"
service bbb-web stop
./build.sh
grails assemble
cd "$BIGBLUEBUTTON_WEB_EXPOLODED_DIR"
jar -xvf ../build/libs/bigbluebutton-0.10.0.war
cp ../run-prod.sh .
cp -R /usr/share/bbb-web /usr/share/bbb-web-old
rm -rf /usr/share/bbb-web/assets/ /usr/share/bbb-web/META-INF/ /usr/share/bbb-web/org/ /usr/share/bbb-web/run-prod.sh  /usr/share/bbb-web/WEB-INF/
cp -R . /usr/share/bbb-web/
chown bigbluebutton:bigbluebutton /usr/share/bbb-web
chown -R bigbluebutton:bigbluebutton /usr/share/bbb-web/assets/ /usr/share/bbb-web/META-INF/ /usr/share/bbb-web/org/ /usr/share/bbb-web/run-prod.sh /usr/share/bbb-web/WEB-INF/
...
lsVar="${array[0]}"
echo $lsVar
readarray -d  -t strarr <<<"$lsVar"
echo ${strarr[3]}
readarray -d / -t strarr2 <<<"${strarr[3]}"
echo HEY
final="bbb-apps-akka_""${strarr2[0]}""_all.deb"
sudo dpkg -i $final
cd "$BIGBLUEBUTTON_HTML5_APP_DIR"
#ARP
...
#ARP
# sudo mv /home/settings.yml "$DESTINATION_DIR"/programs/server/assets/app/config/
# sudo bbb-conf --restart
# sleep 15s
# echo '(ARP) ALL IS WELL :)'
#ARP
