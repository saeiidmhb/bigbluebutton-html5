...
const ChatContainer = (props) => {
  ...
  function checkGame(item) {
    return !item.extra || item.extra.type !== 'game';
  }
  ...
  const applyPropsToState = () => {
    ...
    if (
      ...
      let timeWindowsValues = isPublicChat
        ? [
          ...(
      ...
      if (isPublicChat && timeWindowsValues)
      {
        let foundedPolls = Object.values(timeWindowsValues).filter(checkGame);
        timeWindowsValues = foundedPolls;
      }
  ...
  return (
    <Chat institutionWelcomeMessage={institutionWelcomeMessage} {...{
      idChatOpen,
      isChatLocked,
      ...restProps,
      chatID,
