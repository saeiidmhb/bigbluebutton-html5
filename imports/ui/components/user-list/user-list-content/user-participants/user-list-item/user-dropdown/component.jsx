...
class UserDropdown extends PureComponent {
  ...
  */
  ...
  renderUserAvatar() {
    ...
    let iconUser = null;
    if (user.emoji !== 'none') {
      if (user.emoji === 'inactiveTab' && !inactiveTabDetectionEnabled)
        iconUser = user.name.toLowerCase().slice(0, 2);
      else
        iconUser =<Icon iconName={normalizeEmojiName(user.emoji)} />
    }
    else
      iconUser = user.name.toLowerCase().slice(0, 2);
