...
import GameContainer from '/imports/ui/components/gameT/container';
...
class PanelManager extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (gameSize.width !== oldGameSize.width && gameSize.width !== gameWidth) {
      this.setGameWidth(gameSize.width);
    }
  ...
  gameResizeStop(addvalue) {
    const { gameWidth } = this.state;
    const { layoutContextDispatch } = this.props;

    this.setGameWidth(gameWidth + addvalue);

    layoutContextDispatch(
      {
        type: 'setGameSize',
        value: {
          width: gameWidth + addvalue,
        },
      },
    );
  ...
  renderChat() {
    ...
    const {institutionWelcomeMessage} = this.props;
    ...
    return (
      ...
      >
        <ChatContainer institutionWelcomeMessage={institutionWelcomeMessage}/>
      </section>
    );
  }
  ...
  renderGame() {
    ...
    return (
      <div className={styles.poll} key="action-item-00" id="gamePanel">
  ...
  renderGameResizable() {
    ...
    return (
      <Resizable
        ...
        enable={resizableEnableOptions}
        key="action-item-00"
        size={{ width: gameWidth }}
        onResizeStop={(e, direction, ref, d) => {
          this.gameResizeStop(d.width);
        }}
      >
        {this.renderGame()}
      </Resizable>
  ...
  render() {
    ...
    console.log("openPanelllllllllllll")
    ...
    if (openPanel === 'game') {
      if (enableResize) {
        panels.push(this.renderGameResizable());
      } else {
        panels.push(this.renderGame());
      }
    }
