...
class VideoService {
  ...
  disableCamLimitedCon() {
    const m = Meetings.findOne({ meetingId: Auth.meetingID },
      { fields: { 'lockSettingsProps.disableCamLimitedCon': 1 } });
    return m.lockSettingsProps ? m.lockSettingsProps.disableCamLimitedCon : false;
  }
  ...
  isLimitedConUserLocked() {
    return !!Users.findOne({
      userId: Auth.userID,
      locked: true,
      role: { $ne: ROLE_MODERATOR },
    }, { fields: {} }) && this.disableCamLimitedCon();
  }
  ...
  disableReason() {
    const locks = {
      ...
      videoLockedLimitedCon: this.isLimitedConUserLocked(),
