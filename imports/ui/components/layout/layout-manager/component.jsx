...
class LayoutManagerComponent extends Component {
    ...
    layoutContextDispatch(
      {
        type: 'setGameSize',
        value: {
          width: layoutSizes.gameSize.width,
        },
      },
    );
    layoutContextDispatch(
  ...
  calculatesPanelsSize(panelChanged) {
    ...
    if (storageLData) {
      ...
      storageGameWidth = storageLData.gameSize?.width;
    ...
    if (panelChanged && gameSizeContext.width !== 0) {
      newGameSize = gameSizeContext;
    } else if (!storageGameWidth) {
      newGameSize = {
        width: min(max((windowWidth() * 0.2), GAME_MIN_WIDTH), GAME_MAX_WIDTH),
      };
    } else {
      newGameSize = {
        width: storageGameWidth,
      };
    }
  ...
  calculatesLayout(panelChanged = false) {
    ...
    } else if (newGameSize.width > 0) {
      secondPanel = newGameSize;
