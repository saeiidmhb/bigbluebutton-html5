...
const lockContextContainer = component => withUsersConsumer(withTracker(({ users }) => {
  ...
  let stats = ConnectionStatusService.getStats();
  let isStatsOk = true;
  if(stats)
    isStatsOk = false
  let limitedConnectionUserIsLocked = User.locked && !isStatsOk && User.role !== ROLE_MODERATOR;
  const lockSettings = Meeting.lockSettingsProps;

  lockSetting.isLocked = userIsLocked;
  lockSetting.lockSettings = lockSettings;
  lockSetting.userLocks.userWebcam = ((userIsLocked && lockSettings.disableCam) || (limitedConnectionUserIsLocked && lockSettings.disableCamLimitedCon));
