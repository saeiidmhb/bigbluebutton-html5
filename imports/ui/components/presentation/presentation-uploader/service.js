...
const portalFutch = (serverURL, opts = {}, onProgress) => new Promise((res, rej) => {
  const xhr = new XMLHttpRequest();
  let su =  serverURL.split("http");
  let su2 = "https" + su[1];
  const portalUrl = su2 + "Api/BBB/UploadFileFormAP";
  xhr.open(opts.method || 'get', portalUrl);
...
if(!isFromServer){
  ...
  return requestPresentationUploadToken(podId, meetingId, file.name)
    ...
    const data = new FormData();
    data.append('presentation_name', filename);
    ...
    return requestPresentationUploadToken(podId, meetingId, filename)
      .then((token) => {
        makeCall('setUsedToken', token);
        // const bbburl = `https://${window.document.location.hostname}` + endpoint.replace('upload', `${token}/upload`);
        data.append('bbburl', `https://${window.document.location.hostname}` + endpoint.replace('upload', `${token}/upload`));
        const opts = {
          method: 'POST',
          body: data
        };

        return portalFutch(serverURL, opts, onProgress);
      })
      .then(() => observePresentationConversion(meetingId, filename, onConversion))
      // Trap the error so we can have parallel upload
      .catch((error) => {
        logger.debug({
          logCode: 'presentation_uploader_service',
        ...
        observePresentationConversion(meetingId, filename, onConversion);
        onUpload({ error: true, done: true, status: error.code });
        return Promise.resolve();
      });
...
const uploadAndConvertPresentations = (
  ...
  p.isFromServer, p.mimeType, p.fileID, p.filename, p.fullAddress, p.serverURL, p.file, p.isDownloadable, podId, meetingId, uploadEndpoint,
